    ...
    return <div className="font-poppins">
        ...
        <div className="flex items-center gap-3 text-sm">
            ...
            <p className="text-sm font-normal opacity-80">
                Return possible until (30-09-2025)
        ...
        <div className="flex flex-col gap-6 ">
                        ...
                        <div key={index} className="flex flex-col gap-1">
                            <div className="flex flex-row gap-2 items-center">
                                <input
                                    ...
                                    name="products"
                                    value={item.product}
                                    checked={inputs.products.includes(item.product)}
                                    onChange={() => setInputs({ ...inputs, products: [...inputs.products, item.product] })}
                                    className=" accent-primaryhover"
                                />
                                <label htmlFor={`product-${index}`}>{item.name}</label>
                            </div>
                        ...
                        <div key={index} className="flex flex-col gap-1">
                            <div className="flex flex-row gap-2 items-center">
                                <input
                                    ...
                                    name="returnReason"
                                    value={reason}
                                    checked={inputs.reason === reason}
                                    onChange={() => setInputs({ ...inputs, reason })}
                                    className=" accent-primaryhover"
                                />
                            ...
                            {reason === "Other" && inputs.reason === "Other" && (
                                <div className=" border border-gray-200 w-[400px] h-[80px]">
                                    <textarea
                                        ...
                                        onChange={(e) => setInputs({ ...inputs, otherReason: e.target.value })}
                                    />
                                </div>
                            )}
                        </div>
            ...
            <div className="text-sm flex flex-col gap-3">
                ...
                <div>
                    {lists.itemUsed.map((condition, index) => (
                        <div key={index} className="flex flex-row gap-2 items-center">
                            <input
                                ...
                                value={condition}
                                className=" accent-primaryhover"
                                onChange={() => setInputs({ ...inputs, itemUsed: condition })}
                                checked={inputs.itemUsed === condition}
                            />
            ...
            <div className="text-sm flex flex-col gap-3">
                ...
                <div className="flex  border border-gray-200  h-[40px] rounded-sm overflow-hidden">
                    <span className="px-3 bg-gray-100 text-gray-600 text-sm flex items-center">+234</span>
            ...
            <div className="text-sm flex flex-col gap-3">
                ...
                {/* {imageError && (
            <p className="text-red-500 text-[12px] mt-1">{imageError}</p>
          )} */}
            </div>
        ...
        <div className="flex justify-between mt-5 items-center gap-5">
            <button

                disabled={!(inputs.reason && inputs.itemUsed && inputs.imageEvidence && inputs.phoneNumber)}
                onClick={handleSubmit}
                className={`bg-primaryhover w-[200px] h-[40px] flex items-center justify-center text-[14px] text-white rounded-sm  ${!(inputs.reason && inputs.itemUsed && inputs.imageEvidence && inputs.phoneNumber) ? "opacity-50 cursor-not-allowed" : ""}`}
            >
                {loading ? <ImSpinner8 size={20} className="animate-spin" /> : "Submit Request"}

            </button>
            <Link href={"/pages/ordersandaccount/returns"} className="border border-gray-400 w-[200px] h-[40px] flex items-center justify-center text-[14px] text-black rounded-sm">
                Go to Returns
            </Link>
        </div>
    </div>
...
export default function IssueReturn({ returnModal, setReturnModal, data }: { returnModal: boolean, setReturnModal: (modal: boolean) => void, data: { _id: string, items: IItem[] } }) {

    return (
        <>
            <div className='hidden md:block'>
                <Modal isOpen={returnModal} onClose={() => setReturnModal(false)}>
                    <Children data={data} setReturnModal={setReturnModal} />
                </Modal>
            </div>

            <div className={` ${returnModal ? "md:hidden block" : "hidden "}`}>
                <div className='fixed inset-0 bg-white  py-6 h-screen overflow-y-auto'>
                    <MobileHeader text='Return Request' handleBack={() => setReturnModal(false)} />
                    <div className="px-4">
                        <Children data={data} setReturnModal={setReturnModal} />
                    </div>
                </div>
