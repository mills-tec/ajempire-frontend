...
export default function AboutTab() {
    ...
    const tabs = [
        { name: "About Us", path: "/pages/ordersandaccount/support/about" },
        { name: "About this App", path: "/pages/ordersandaccount/support/aboutthisapp" },
    ];
    return (
        <div className="">
            ...
            <ul className="w-full flex items-center justify-between lg:justify-start lg:gap-10 font-poppins lg:text-[14px] text-[12px]">
                {tabs.map((tab) => (
                    <li key={tab.path}>
                        <Link
                            href={tab.path}
                            className={`relative pb-2 transition-colors lg:mr-5  duration-200 ${pathname === tab.path
                                ? "text-primary"
                                : "text-[#525252] hover:text-primary"
                                }`}>
                            {tab.name}
                            {pathname === tab.path && (
                                <span className={` absolute   -translate-x-6  bottom-0 w-5 border-b-4 border-primaryhover rounded-full ${pathname === "/pages/ordersandaccount/support/about" ? "lg:left-10 left-10" : pathname === "/pages/ordersandaccount/support/aboutthisapp" ? "lg:left-14 left-7" : ""}`}>
                                </span>
                            )}
                        </Link>
                    </li>
