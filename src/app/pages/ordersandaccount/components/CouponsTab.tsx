...
export default function CouponsTab() {
    ...
    const tabs = [
        { name: "Unused", path: "/pages/ordersandaccount/coupoonsandoffers" },
        { name: "Used", path: "/pages/ordersandaccount/coupoonsandoffers/usedcoupon" },
                ...
                {tabs.map((tab) => (
                    <li key={tab.path}>
                        <Link
                            ...
                            {tab.name}
                            {pathname === tab.path && (
                                <span className={` absolute   -translate-x-1/2  bottom-0 w-5 border-b-4 border-primaryhover rounded-full ${pathname === "/pages/ordersandaccount/coupoonsandoffers/usedcoupon" ? "lg:left-4 left-4" : "lg:left-6 left-6"}`}>
                                </span>
                            )}
                        </Link>
                    </li>
