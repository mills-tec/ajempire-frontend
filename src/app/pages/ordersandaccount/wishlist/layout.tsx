...
export default function WishlistLayout({
    ...
    const isProducts = pathname === "/pages/ordersandaccount/wishlist" || pathname === "/wishlist/products";
    const isUpdates = pathname === "/pages/ordersandaccount/wishlist/updatewishlist";

    return (
        <div className="space-y-4 px-6 h-[520px] overflow-y-scroll">
            <div className="font-poppins font-[14px] flex flex-col gap-4">
                <div className="lg:hidden flex items-center  mb-[10px]">
                    ...
                    <div className=" w-full text-center font-semibold text-black/70">
                        <p>Your Wish / Watch-List</p>
                    </div>

                </div>
                <div className=" flex items-center justify-around  w-full gap-6  mb-[15px] lg:mt-0">
                    <button
                        onClick={() => router.push("/pages/ordersandaccount/wishlist")}
                        className={clsx(
                            "pb-2 text-sm",
                            isProducts && "border-b-4 border-brand_pink text-brand_pink font-medium"
                        )}>
                        Products
                    </button>
                    <button
                        onClick={() => router.push("/pages/ordersandaccount/wishlist/updatewishlist")}
                        className={clsx(
                            "pb-2 text-sm",
                            isUpdates && "border-b-4 border-brand_pink text-brand_pink font-medium"
                        )}
                    >
                        Updates
                    </button>
