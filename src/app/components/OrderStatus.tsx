...
const ClockIcon = ({ active }: { active: boolean }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={`size-5 ${active ? "fill-[#0085FF]" : "fill-black/45"}`}>
    ...
    <defs><clipPath id="os-clock"><rect width="24" height="24" fill="white" /></clipPath></defs>
  </svg>
);

const SpinnerIcon = ({ active }: { active: boolean }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" className={`size-5 ${active ? "fill-[#FF8D28]" : "fill-black/45"}`} xmlns="http://www.w3.org/2000/svg">
    <circle cx="12" cy="12" r="12" />
    <path d="M12 4.5V7M12 17V19.5M19.5 12H17M7 12H4.5M17.3033 6.6975L15.535 8.465M8.465 15.535L6.6975 17.3025M17.3033 17.3033L15.535 15.535M8.465 8.465L6.6975 6.6975" stroke="white" strokeWidth="1.5" strokeLinecap="round" />
  </svg>
);
...
const TruckIcon = ({ active }: { active: boolean }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" className={`size-5 ${active ? "fill-purple-600" : "fill-black/45"}`} fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <defs><clipPath id="os-truck"><rect width="16" height="16" fill="white" transform="translate(4 4)" /></clipPath></defs>
  </svg>
);
...
export default function OrderStatus({ createdAt, processedAt, shippedAt, deliveredAt,  trackingUrl}: IOrderStatus) {
  ...
  const steps = [
    {
      ...
      desc: "Thank you, your order has been received",
      Icon: ClockIcon,
      activeColor: "text-[#0085FF]",
    },
    {
      date: processedAt,
      label: processedAt && shippedAt ? "Order Processed" : "Order Processing",
      desc: "Your order is being processed",
      Icon: SpinnerIcon,
      activeColor: "text-[#FF8D28]",
    },
    {
      date: shippedAt,
      label: "Order Shipped",
  ...
  return (
    <div className="mt-8 w-max font-poppins">
      {steps.map((step, i) => {
