...
export default function PaymentMethod({
  ...
  const _flutterWaveLogo = "https://logotyp.us/file/flutterwave.svg";
  ...
  const handleContinue = () => {
    ...
    (async () => {
      try {
        const logisticsStatus = await checkLogistics();
        if (logisticsStatus) {
          onNext();
        } else {
          skipLogistics();
  ...
  // checking if logistics is on if its not we skip the logistics step
  const checkLogistics = async () => {
    const req = await axios.get(`${globalUrl}/logisticsStatus`);

    return req.data.logisticsMode === "auto";
  };
  return (
    <div className="fixed inset-0 bg-[#FFFFFF] flex lg:items-center lg:justify-center z-50">
      ...
      <div className="w-full relative shadow-lg font-poppins text-[14px] lg:w-[50%] lg:h-[500px] lg:px-10 px-5 py-8">
        <p className="text-center font-semibold opacity-75 mb-6">
          Payment Method
        </p>
        ...
        <div className="lg:px-[30px]">
          ...
          <div className="space-y-4">
            {/* Paystack Option */}
            <div
              className={`flex items-center justify-between border px-6 py-3 rounded-lg cursor-pointer ${
                selectedPaymentMethod === "paystack"
                  ? "border-primaryhover bg-pink-50"
                  : "border-gray-300"
              }`}
            ...
            <button
              ...
              className={`w-full h-[35px] rounded-md text-white ${
                selectedPaymentMethod
                  ? "bg-primaryhover hover:bg-[#E6007E]"
