...
export default function DraggableCartButton({ cartRef, itemCount }: Props) {
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    setPos({
      ...
      y: Math.max(
        80,
        Math.min(window.innerHeight - SIZE - 160, startPosRef.current.y + dy),
      ),
    });
  };
  ...
  const onPointerUp = () => {
    ...
    if (hasMovedRef.current) {
      setPos((prev) => {
        if (!prev) return prev;
        const snapLeft = prev.x + SIZE / 2 < window.innerWidth / 2;
        return {
          x: snapLeft ? EDGE_MARGIN : window.innerWidth - SIZE - EDGE_MARGIN,
          y: prev.y,
        };
      });
    } else {
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: isDragging
          ? "none"
          : "left 0.3s cubic-bezier(0.34, 1.56, 0.64, 1), top 0.15s ease",
      }}
    ...
    >
      ...
      >
        ...
        {itemCount > 0 && (
          <div className="absolute size-5 rounded-full -top-1 -right-1 z-10 bg-brand_pink text-white text-xs font-semibold flex items-center justify-center">
            {itemCount}
          </div>
        )}
      </Link>
